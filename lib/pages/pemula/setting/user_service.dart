...
class UserService {
  static Future<http.Response> updateProfile(int userId, String username, String password) async {
  final token = await _getToken();
  final url = Uri.parse('${ApiConfig.baseUrl}/update-profile/$userId');

  print('Sending to: $url'); // Debug URL
  print('Payload: {"username": "$username", "password": "$password"}'); // Debug payload
  ...
  try {
    final response = await http.put(
      ...
      body: jsonEncode({
        'username': username,
        'password': password.isNotEmpty ? password : null, // Kirim null jika password kosong
      }),
    );
